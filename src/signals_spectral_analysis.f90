...
! ------------------------------------------------------------------------------
    !
    subroutine overlap_p2(x, window, winsize, avg, avgpow)
        ! Arguments
        real(real64), intent(in), dimension(:) :: x
        procedure(window_function), intent(in), pointer :: window
        integer(int32), intent(in) :: winsize
        real(real64), intent(out), dimension(:), optional :: avg, avgpow

        ! Parameters
        real(real64), parameter :: two = 2.0d0

        ! Local Variables
        integer(int32) :: i, lwsave, lwork, flag
        real(real64) :: ndp
        real(real64), allocatable, dimension(:) :: wsave, work, s

        ! Quick Return
        if (.not.present(avg) .and. .not.present(avgpow)) return

        ! Ensure winsize is an integer power of two

        ! Initialization
        ndp = real(winsize, real64)
        lwork = winsize
        lwsave = lwork + int(log(ndp) / log(two), int32) + 4
        allocate(work(lwork))
