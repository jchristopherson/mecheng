...
interface
    ...
    !!  If supplied, the routine determines the optimal size for @p work, and
    !!  returns without performing any actual calculations.
    !! @param[out] rwork An optional input, that if provided, prevents local
    !!  memory allocation for a real-valued workspace array.  If provided, the
    !!  length of the array must be 2*N elements in length.
    !! @param[out] err An optional errors-based object that if provided can be
    !!  used to retrieve information relating to any errors encountered during
    !!  execution.  If not provided, a default implementation of the errors
    !!  class is used internally to provide error handling.  Possible errors and
